...
import { allDATA} from '../store/action/supervisor';


export default function Supervisor() {
    const disptach = useDispatch();
    const supervisor = useSelector(state => state.supervisor.data, shallowEqual)
    ...
    const column = [
        ...
        },
        {
            ...
            render : (value,index) =>{
                const menuDropdown = (
                    <Menu>
                      <Menu.Item key={1} onClick={()=>openDrawer(index)}>View Supervisor Info</Menu.Item>
                      <Menu.Item key={2} ><Link to={'/collectorList/'+value}>View Collectors</Link></Menu.Item>
                    </Menu>
                  );
                  return(
        ...
        <>
            <div className="create-form-cont bg-white mt-4">
                <div className='d-flex justify-content-between'>
                    <h2 className='text-center sub-heading mb-4'>Supervisors</h2>
                    <Link to="/create_supervisor"><Button type="primary">Create Supervisor</Button></Link>
                </div>
