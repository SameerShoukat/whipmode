...
export function allDATA() {
  return async (dispatch) => {
    ...
    try {
      ...
      const res = await axios.get(`${api}/supervisor/my-complains`, config);
...
export const closeRequest = (value) => {
  const config = {
    headers: {
      authorization: `Bearer ${localStorage.getItem("uswms-login")}`,
    },
  ...
  axios
    .put(`${api}/supervisor/change-complain-status/${value}`, null, config)
    .then((res) => {
      if (res) {
        return res.message;
      }
